...
from ex1_0_utils import LogisticDistribution


"""
ConditionalRealNVP coupling layer
"""
class ConditionalCouplingLayer(nn.Module):
    ...
    def __init__(self, input_dim, output_dim=None, hidden_dim=256, mask=None, num_layers=4,
        ...
        modules = [nn.Linear(input_dim+num_classes, hidden_dim), 
                    nn.LeakyReLU(0.2)]
    ...
    def forward(self, z, labels):
        ...
        z_A_cond = torch.cat([z_A, F.one_hot(labels, self.num_classes)], dim=-1)
        ms_z_A = self.ms(z_A_cond)
...
class ConditionalRealNVP(nn.Module):
    def __init__(self, input_dim, hidden_dim=256, num_coupling_layers=3, num_layers=6, device='cpu', 
        ...
        self.coupling_layers = nn.ModuleList([ConditionalCouplingLayer(input_dim=input_dim,
    ...
    def forward(self, z, labels):
        ...
        for i in range(len(self.coupling_layers)):  # pass through each coupling layer
            x = self.coupling_layers[i](x, labels)
        return x
    ...
    def inverse(self, x, labels):
        ...
        for i in reversed(range(len(self.coupling_layers))):  # pass through each coupling layer in reversed order
            z, log_det_jacobian = self.coupling_layers[i].inverse(z, labels)
    ...
    def sample(self, labels):
        z = self.prior.sample([len(labels), self.input_dim]).view(len(labels), self.input_dim)
        z = z.to(self.device)
...
"""
"""
def generate_from_model(model, labels=None, savepath=None):
    ...
    for row in ax:
        for col in row:
            ...
            col.axis('off')
            col.set_title(f'Label: {labels[i].item()}')
            i += 1
